...
const OUT_DIR = "/Users/armanyeghiazaryan/byvaro-design-armenia/screenshots/emails-final-validation";
const VIEWPORTS = [
...
async function block1Toaster(browser) {
  ...
  try {
    ...
    r.steps.push(`Toaster global montado en /emails: ${mounted}`);
    if (!mounted) { r.pass = false; r.errors.push("Toaster global NO montado en /emails"); }
...
async function block3Drafts(browser) {
  ...
  try {
    ...
    // Cerrar con X del header — tercer boton en header (minimize, expand, close)
    const xClicked = await page.evaluate(() => {
      ...
      // Buscar botón con icono lucide-x (cerrar)
      const btns = [...compose.querySelectorAll("button")];
      const xBtn = btns.find((b) => b.querySelector("svg.lucide-x"));
      if (xBtn) { xBtn.click(); return { ok: true }; }
      return { ok: false, reason: "no X button" };
    });
    ...
    const rowClicked = await page.evaluate(() => {
      // Buscar fila que contenga "Borrador" como subject dentro de la lista
      const cands = [...document.querySelectorAll("div")];
      const row = cands.find((el) => {
        const t = (el.textContent || "").trim();
        const cls = (el.className || "").toString();
        return cls.includes("cursor-pointer") && /Borrador/.test(t);
      });
...
async function block4ComposeRedesign(browser) {
  ...
  try {
    ...
    await page.locator('input[placeholder="Para"]').first().fill("test@gmail.com");
    await page.keyboard.press("Enter");
    await page.waitForTimeout(300);

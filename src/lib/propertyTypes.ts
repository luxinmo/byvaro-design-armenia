...
const propertyTypeAliases: Record<string, string> = {
  ...
  /* Wizard unifamiliar (raw ids del usuario) → labels en inglés
   *  legacy que el listado entiende. Los ids raw NO se normalizan a
   *  inglés · permanecen como `independiente` / `adosados` /
   *  `pareados` y solo se convierten a label visible al renderizar. */
};

/**
 * MultimediaStep · Paso "Fotografías + Videos" del wizard Crear Promoción.
 *
 * Fotografías:
 *   - Subida (mock: añade 3 imágenes desde Unsplash al confirmar).
 *   - Grid 3 cols con reordenación por drag & drop.
 *   - Por foto: nombre, categoría, marcar como principal, bloquear
 *     exportación, borrar.
 *   - Badge "PRINCIPAL" + badge "Bloqueada".
 *   - Warning si el promotor tiene `colaboracion = true` avisando del
 *     tema marcas de agua y exportación a agencias.
 *
 * Videos:
 *   - 3 tipos: YouTube (URL), subida directa MP4 mock, 360° Vimeo.
 *   - Lista vertical con icono + nombre + URL + borrar.
 *
 * Port adaptado de figgy-friend-forge/src/components/create-promotion/
 * StepMultimedia.tsx — sin shadcn, modales reemplazados por un shell
 * ligero fixed-inset-0 con tokens Byvaro.
 */
...
import type { WizardState, FotoItem, FotoCategoria, VideoItem } from "./types";
...
export function MultimediaStep({
  ...
  const addMockPhotos = () => {
    ...
    update("fotos", [...state.fotos, ...newPhotos]);
    setUploadOpen(false);
  };

  const setPrincipal = (id: string) =>
    update("fotos", state.fotos.map((f) => ({ ...f, esPrincipal: f.id === id })));

  const toggleLock = (id: string) =>
    update("fotos", state.fotos.map((f) => (f.id === id ? { ...f, bloqueada: !f.bloqueada } : f)));

  const setCategoria = (id: string, cat: FotoCategoria) =>
    update("fotos", state.fotos.map((f) => (f.id === id ? { ...f, categoria: cat } : f)));

  const removePhoto = (id: string) =>
    update("fotos", state.fotos.filter((f) => f.id !== id));

  const handleDragOver = (e: React.DragEvent, idx: number) => {
    e.preventDefault();
    if (dragIdx === null || dragIdx === idx) return;
    const next = [...state.fotos];
    const [moved] = next.splice(dragIdx, 1);
    next.splice(idx, 0, moved);
    update("fotos", next.map((f, i) => ({ ...f, orden: i })));
    setDragIdx(idx);
  };
  ...
  const addVideo = () => {
    ...
    update("videos", [...state.videos, newVideo]);
    setVideoUrl("");
    setVideoNombre("");
    setVideoOpen(false);
  };
    ...
    <div className="flex flex-col gap-5">
      ...
      <div className="flex flex-col gap-3">
        <div className="flex items-center justify-between">
          <p className="text-[11px] font-semibold uppercase tracking-[0.12em] text-muted-foreground">Fotografías</p>
          <button
            type="button"
      ...
      <div className="flex flex-col gap-3">
        <div className="flex items-center justify-between">
          <p className="text-[11px] font-semibold uppercase tracking-[0.12em] text-muted-foreground">Videos</p>
          <button
            type="button"

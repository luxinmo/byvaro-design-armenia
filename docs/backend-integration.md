...
### 7.3 · Oportunidades (pipeline comercial)

**Tipo**: `src/data/opportunities.ts` → `Opportunity`. Consumidores:
`src/pages/Oportunidades.tsx`, `src/pages/OportunidadDetalle.tsx`.
Spec UI en `docs/screens/oportunidades.md` + `oportunidad-detalle.md`.

#### Diferencia con Lead / Registro

Una oportunidad es un **cliente cualificado** en pipeline comercial —
distinto de un lead (entrada cruda) y de un registro (alta del
cliente ante el promotor vía agencia). Una oportunidad puede tener
0..N registros (un mismo cliente puede ser registrado en varias
promociones) y los registros viven **dentro** de la ficha de la
oportunidad como bloque colapsable.

#### CRUD principal

```
GET    /api/opportunities                  → Opportunity[] (paginado, con filtros)
GET    /api/opportunities/:id              → Opportunity (full: matches + registrations + timeline + comments + emails)
POST   /api/opportunities                  body: OpportunityCreateInput → Opportunity
PATCH  /api/opportunities/:id              body: Partial<Opportunity>
DELETE /api/opportunities/:id              (archive · NO hard delete)
```

**Filtros del listado** (query params):

- `stage`, `assignee`, `promotion`, `nationality`, `temperature`,
  `source`, `createdFrom`, `createdTo`, `lastActivityFrom`,
  `lastActivityTo`, `hasPendingRegistration`, `hasAcceptedRegistration`,
  `hasVisit`, `inactiveSinceDays`, `hasEmailBounce`, `hasEmailOpen`,
  `propertyType`, `area`, `budgetMin`, `budgetMax`, `cursor`, `limit`, `q`.

**Visibilidad**:
- Agencias **no** ven oportunidades (ruta oculta · backend responde 403
  si una agencia la pide). Es una entidad del promotor.
- Miembros con `opportunities.viewOwn` → filtrar por `assigneeUserId`.

#### Sub-recursos

```
POST   /api/opportunities/:id/stage             { stage }
                                                → actualiza `stage` + emite `stage-changed` en timeline.
                                                Si stage === "ganada" o "perdida", bloquea ediciones posteriores.

GET    /api/opportunities/:id/matches           → OpportunityMatch[]   (ver §7.3.1 · IA)

POST   /api/opportunities/:id/registrations     body: { promotionId, agencyId?, agentId, note? }
                                                → crea OpportunityRegistration con status="pendiente".
                                                Emite `registration-created` en timeline.

PATCH  /api/opportunities/:id/registrations/:rid { status: "aceptado"|"rechazado"|"cancelado", note? }
                                                → muta status, registra `decidedAt` + `decidedByName`.
                                                Emite `registration-accepted`/`registration-rejected`.

POST   /api/opportunities/:id/visits            body: { scheduledAt, unit?, property?, attendees[] }
                                                → crea visita + auto-mueve stage a "visita" si estaba en "interes".

POST   /api/opportunities/:id/visits/:vid/evaluate  body: { outcome, rating?, note? }
                                                → evalúa visita realizada + permite auto-mover a "evaluacion".

GET    /api/opportunities/:id/comments          → OpportunityComment[]
POST   /api/opportunities/:id/comments          body: { body } (usuario = JWT)

GET    /api/opportunities/:id/emails            → OpportunityEmailEvent[] (resumen)
POST   /api/opportunities/:id/emails            body: { to, subject, body, attachments[] }
                                                → enviar + trackear `delivered/opened/bounced/replied` (webhooks).

GET    /api/opportunities/:id/timeline          → OpportunityTimelineEvent[] (append-only)
```

#### Conversión Lead → Oportunidad

```
POST /api/leads/:id/convert-to-opportunity    body: { stage? }   (default: settings.defaultInitialStage)
                                              → Opportunity
```

Debe hacer **atómicamente**:
1. Crear `Opportunity` con `stage = body.stage ?? settings.defaultInitialStage`, `source="lead"`, `sourceLeadId = leadId`.
2. Copiar `lead.interest` → `opportunity.interest` (promoción origen incluida).
3. Copiar timeline del lead (con `type="lead-received"` al principio).
4. Copiar emails + comentarios del lead en la oportunidad.
5. Marcar `lead.status = "converted"`.
6. Emitir `opportunity-created` en el timeline de la oportunidad.
7. Emitir evento en el historial del contacto (regla `CLAUDE.md §🥇`).

Trigger UI: `src/pages/LeadDetalle.tsx:ConvertToOpportunityDialog` →
`onConfirm()`.

#### Settings · `/ajustes/leads-oportunidades`

```
GET   /api/settings/leads-opportunities  → { flowMode, defaultInitialStage, showRegistrationsInOpp, showPendingRegBadge, leadConversion }
PATCH /api/settings/leads-opportunities  body: Partial<...>
```

Mock actual: `localStorage.byvaro.leadsOpps.settings.v1` via
`src/pages/ajustes/leads-oportunidades.tsx`.

Efectos de cada setting:
- `flowMode: "lead-to-opp" | "direct-to-opp"` · elige si los formularios
  web / portales crean Lead (default) u Oportunidad directa.
- `defaultInitialStage` · etapa de entrada (Interés | Visita |
  Evaluación | Negociación).
- `showRegistrationsInOpp` · oculta el bloque de registros en la ficha
  (empresas que no registran vía agencia).
- `showPendingRegBadge` · apaga el aviso en lista + cabecera.
- `leadConversion: "manual"` · V1 fijo. Queda hueco para `"auto"` con
  reglas en el futuro.

#### §7.3.1 · Matching con IA

```
GET /api/opportunities/:id/matches?limit=6  → OpportunityMatch[]
POST /api/opportunities/:id/matches/generate → OpportunityMatch[]   (regenera usando IA)
```

Inputs (lado backend) para el scoring: `interest.propertyType`, `area`,
`budgetMin/Max`, `bedrooms`, `extras[]`, `originPromotionId`, más
actividad del contacto (páginas vistas, emails respondidos). Output
ordenado por `score` descendente.

#### Regla de oro cross-historial

Cada mutación sobre una oportunidad emite **dos** eventos:
- Uno en `opportunity.timeline` (auditoría propia).
- Otro en `contact.history` del contacto dueño (regla §🥇 de `CLAUDE.md`).

Esto permite que desde la ficha del contacto se vea el resumen de lo
que pasó en sus oportunidades sin tener que entrar a cada una.
